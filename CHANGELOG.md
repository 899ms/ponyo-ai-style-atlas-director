...
## V3.1.1 — Quickstart & Copyright-Safe Translation Notes（2026-07）

### Added
- 在 README 增加“下载后怎么用”章节：给出三条触发句、完整使用流程，以及“先安全转译，再生成封面/故事板/视频提示词”的工作逻辑。
- 在 README 和 SKILL 中增加“版权安全转译机制”说明：强调传播版名称只用于理解用户意图，生产提示词不写导演名、工作室名、角色名或具体 IP 名，只保留构图、色彩、光影、材质、情绪等视觉 DNA。
- 在 SKILL 中增加 Default Usage Flow，帮助下载后的 Agent 按“用户故事/题材 → 叙事骨架 → 参考氛围 → 商用安全转译 → 视觉 DNA → 输出模式 → 引擎编译 → 质检”的顺序执行。
- 明确“用户自己的故事”是故事板流程的前置输入：先收集或补齐故事梗概、主角、目标、阻碍、转折、结尾定格，再进入风格转译和六宫格/八宫格生成。
- 升级 `templates/storyboard_template.md`，加入用户故事、叙事骨架和“每格必须推进故事，不能只是氛围摆拍”的约束。

### Clarified
- 明确本 Skill 不是教用户绕过审核规则，而是在生成前主动清理高风险版权元素，把模仿请求改写成更安全、更可解释的艺术指导语言。
